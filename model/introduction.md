...
- \(R_g\): Ground reference frame.
- \(R_i\) is the reference frame attached to link \(L_i\), with its origin located at the end of the link.
...
- Any point in space, defined in n link reference frame.
```math
{}^{n}X
```

- i link velocity, defined in n link reference frame.
```math
{}^{n}v_i
```

- i link velocity first derivative, defined in n link reference frame.
```math
{}^{n}\dot{v_i}
```
